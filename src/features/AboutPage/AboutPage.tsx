  ...
  return (
    <div>
      ...
      <Reveal as="section" className={styles.tribute}>
        <p className="eyebrow">
          {lang === 'sv' ? 'Kunskap / Berättelser / Respekt' : lang === 'fr' ? 'Savoir / Histoires / Respect' : lang === 'fi' ? 'Tieto / Tarinat / Kunnioitus' : lang === 'es' ? 'Conocimiento / Historias / Respeto' : lang === 'pt' ? 'Conhecimento / Histórias / Respeito' : lang === 'ko' ? '지식 / 이야기 / 존중' : lang === 'ja' ? '知識 / 物語 / 敬意' : lang === 'zh' ? '知识 / 故事 / 尊重' : 'Knowledge / Stories / Respect'}
        </p>
      ...
      <Reveal as="section" className={styles.continuation}>
        <p className="eyebrow eyebrow-dark">
          {lang === 'sv' ? 'Framåt / Fortsättning' : lang === 'fr' ? 'La suite / L’avenir' : lang === 'fi' ? 'Jatkoa / Tulevaisuus' : lang === 'es' ? 'Mirando al futuro / Continuará' : lang === 'pt' ? 'Olhando para o futuro / Continua' : lang === 'ko' ? '앞으로 / 계속되는 이야기' : lang === 'ja' ? 'これから / 続いていく物語' : lang === 'zh' ? '未来 / 故事继续' : 'Looking ahead / To be continued'}
        </p>
