    ...
    <div>
      <Reveal as="section" className={styles.intro}>
        <p className="eyebrow eyebrow-dark">
          {lang === 'sv' ? 'Källkarta / 1970–2007' : lang === 'fr' ? 'Diagramme source / 1970–2007' : lang === 'fi' ? 'Lähdekaavio / 1970–2007' : lang === 'ko' ? '자료 차트 / 1970–2007' : 'Source map / 1970–2007'}
        </p>
        <h2>{copy.introTitle}</h2>
        {copy.intro.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}
        <div className={styles.legend} aria-label={lang === 'sv' ? 'Förklaring' : lang === 'fr' ? 'Légende' : lang === 'fi' ? 'Selite' : lang === 'ko' ? '범례' : 'Legend'}>
