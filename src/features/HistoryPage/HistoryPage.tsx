    ...
    <div>
      ...
      <Reveal as="section" className={`${styles.panel} ${styles.world}`}>
        <p className="eyebrow eyebrow-dark">
          {lang === 'sv' ? 'Los Angeles → världen' : lang === 'fr' ? 'Los Angeles → le monde' : lang === 'fi' ? 'Los Angeles → maailma' : lang === 'ko' ? 'Los Angeles → 세계' : lang === 'ja' ? 'Los Angeles → 世界' : lang === 'zh' ? 'Los Angeles → 世界' : 'Los Angeles → the world'}
        </p>
      ...
      <Reveal as="section" className={`${styles.panel} ${styles.context}`}>
        <p className="eyebrow">
          {lang === 'sv' ? 'Erkännande / Kontext' : lang === 'fr' ? 'Reconnaissance / Contexte' : lang === 'fi' ? 'Tunnustus / Konteksti' : lang === 'ko' ? '크레딧 / 맥락' : lang === 'ja' ? 'クレジット / 文脈' : lang === 'zh' ? '贡献 / 背景' : 'Credit / Context'}
        </p>
